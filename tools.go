...
const randomStrongSource = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_+"
...
func (t *Tools) RandomString(n int) string {
	s, r := make([]rune, n), []rune(randomStrongSource)
	for i := range s {
...
// UploadFiles uploadas one or more file to a specified directory, and gives the files a random name.
...
func (t *Tools) ReadJSON(w http.ResponseWriter, r *http.Request, data interface{}) error {
	...
	if err != nil {
		...
		case errors.As(err, &syntaxError):
			return fmt.Errorf("body contains badly-formed JSON (at characters %d)", syntaxError.Offset)
		...
		case err.Error() == "http: request body too large":
			return fmt.Errorf("body must not be largr than %d bytes", maxBytes)
...
// WriteJSON takes a response status code an arbitrary data and writes json to the client
...
func (t *Tools) ErrorJASON(w http.ResponseWriter, err error, status ...int) error {
